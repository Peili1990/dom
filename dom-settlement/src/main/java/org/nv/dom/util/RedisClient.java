...
//		shardedJedis = shardedJedisPool.getResource();
	 ...
	 * @author: z```s
	 */
	private void initialPool(String host, int port, String name) {
		// 池基本配置
...
//	}
	...
	public boolean existsHSet(String domain, String key) {
		return jedis.hexists(domain, key);
	}
...
//		Jedis jedis = jedis.getShard(domain);
...
//	}
	...
	public Set<String> getSoredSet(String key, long startScore, long endScore, boolean orderByDesc) {
		...
		if (orderByDesc) {
			values = jedis.zrevrangeByScore(key, endScore, startScore);
		} else {
			values = jedis.zrangeByScore(key, startScore, endScore);
		}
		return values;
	}
	...
	public Set<String> getSoredSetByRange(String key, int startRange, int endRange, boolean orderByDesc) {
		Set<String> values = null;
		if (orderByDesc) {
			values = jedis.zrevrange(key, startRange, endRange);
		} else {
			values = jedis.zrange(key, startRange, endRange);
		}
		return values;
	}
